...
public class MigrationExecutor {
  ...
  /**
   * The datasource to use for migrations. This is injected by a Service Tracker in
   * MigrationMonitoringDSThread.
   */
  ...
  public void enqueue(QChangeLogEntry changeLogEntry) {
    ...
    if (queue.contains(changeLogEntry)) {
      LOGGER.log(Level.FINE, "Bundle {0} is already queued.",
        changeLogEntry.getBundle().getSymbolicName());
    } else {
      queue.add(changeLogEntry);
      LOGGER.log(Level.FINE, "Bundle {0} successfully queued.",
        changeLogEntry.getBundle().getSymbolicName());
    }
  }
  ...
  public void dequeue(String bundleSymbolicName) {
    if (queue.contains(bundleSymbolicName)) {
      ...
      LOGGER
        .log(Level.FINEST, "Can not dequeue bundle not already queued: {0}.", bundleSymbolicName);
    }
  }
  ...
  public void run() {
    ...
    if (settings.getDatasource() != null) {
      ...
      if (MigrationExecutor.getDataSource() != null) {
        ...
        if (terminate) {
          LOGGER
            .log(Level.SEVERE, "Datasource could not be injected because thread was terminated.");
        } else {
          LOGGER
            .log(Level.SEVERE, "Datasource could not be injected because it could not be found.");
        }
      }
    } else {
    ...
    if (!terminate) {
      switch (settings.getMigrationExecutionStrategy()) {
        case START_LEVEL:
          ...
          while (!terminate && MigrationExecutor.getCurrentStartLevel() < settings
            .getBootCompleteSL()) {
            try {
              Thread.sleep(loopDelay);
            } catch (InterruptedException e) {
          ...
          if (MigrationExecutor.getCurrentStartLevel() == settings.getBootCompleteSL()) {
            ...
            LOGGER.log(Level.SEVERE, "SL{0} was not reached, terminating thread.",
    ...
    if (!terminate && Arrays.asList(MigrationExecutionStrategy.ASAP, START_LEVEL)
      .contains(settings.getMigrationExecutionStrategy())) {
    ...
    LOGGER.log(Level.INFO,
      "MigrationAgent thread terminated. Only manual migrations can now be executed.");
    if (liquibaseBootMigrationsDoneServiceServiceRegistration == null) {
      registerMigrationsDoneService();
    }
  }
