...
public class QChangeLogRemoveListener implements EventHandler {
  ...
  public void handleEvent(Event event) {
    if (settings.getDiscoveryMode().equals(MigrationDiscoveryMode.AUTO)) {
      ...
      final String liquibaseChangeLogHeader = bundle.getHeaders()
        .get(Settings.HEADER_Q_LIQUIBASE_CHANGELOG);

      if (liquibaseChangeLogHeader != null) {
        LOGGER.log(Level.FINE, "Removing bundle with Liquibase changelogs: {0}.",
          bundle.getSymbolicName());
        bundleProcessor.unregisterBundle(bundle);
      } else {
        LOGGER.log(Level.FINEST,
          "Bundle {0} ignored for dequeuing since it does not contain a {1} header.",
