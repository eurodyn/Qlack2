...
public class OperationServiceImpl implements OperationService {
	...
	@Override
  public Boolean isPermittedForGroupByResource(String groupID, String operationName, String resourceName, String resourceObjectId) {
    LOGGER.log(Level.FINEST,
      "Checking permissions for group {0}, operation {1} and resource with name {2} and object ID {3}.",
      new String[] { groupID, operationName, resourceName, resourceObjectId});

    Group group = Group.find(groupID, em);

    Boolean retVal = null;
    GroupHasOperation gho = GroupHasOperation
      .findByGroupIDAndOperationNameAndResourceName(groupID, operationName, resourceName,
        resourceObjectId, em);
    if (gho != null) {
      retVal = !gho.isDeny();
    }
