...
public class CacheServiceImplTest {
	...
	// Memcached version.
	private static final String MEMCACHED_IMAGE = "memcached:1.4.25";
	...
	public static void setUpBeforeClass() throws Exception {
		...
		docker.pullImageCmd(MEMCACHED_IMAGE).exec(new PullImageResultCallback()).awaitSuccess();

		// Start containers.
		ExposedPort tcp11211 = ExposedPort.tcp(11211);
		randomLocalPort = NetUtils.getAvailablePort();
		Ports portBindings = new Ports();
		portBindings.bind(tcp11211, Ports.Binding(randomLocalPort));
		container = docker.createContainerCmd(MEMCACHED_IMAGE)
				.withExposedPorts(ExposedPort.tcp(11211))
				.withPortBindings(portBindings)
				.exec();
		docker.startContainerCmd(container.getId()).exec();

		// Setup cache service.
		cache = new CacheServiceImpl(randomLocalPort,
				NetUtils.getURIHost(dockerHost), "127.0.0.1", 0, true);
	}
