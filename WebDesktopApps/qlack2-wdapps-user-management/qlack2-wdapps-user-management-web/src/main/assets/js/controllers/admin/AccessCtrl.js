angular.module('userManagement').controller('AccessCtrl', [
  '$scope', '$state', '$stateParams', 'WindowService', 'ConfigHttpService', '$q', '$timeout', function($scope, $state, $stateParams, WindowService, ConfigHttpService, $q, $timeout) {
    $scope.subjectsDataSource = new kendo.data.DataSource({
      transport: {
        read: function(options) {
          var success;
          return $q.all([ConfigHttpService.getManagedUsers(), ConfigHttpService.getManagedGroups()]).then(success = function(arg) {
