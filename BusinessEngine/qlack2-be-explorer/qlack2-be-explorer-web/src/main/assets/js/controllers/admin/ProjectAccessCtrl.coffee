angular
	.module('explorer')
	.controller 'ProjectAccessCtrl', ['$scope', '$state', '$stateParams', 'WindowService', 'ConfigHttpService', '$q', '$timeout', \
	($scope, $state, $stateParams, WindowService, ConfigHttpService, $q, $timeout) ->
		...
		$scope.subjectsDataSource = new kendo.data.DataSource(
			transport:
				read: (options) ->
					$q.all([ConfigHttpService.getManagedUsers($stateParams.projectId), ConfigHttpService.getManagedGroups($stateParams.projectId)]).then(
